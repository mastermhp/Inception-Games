...
export default function GoogleLoginButton({ onSuccess, onError, className = '' }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`flex items-center justify-center gap-2 bg-white hover:bg-gray-100 text-gray-900 font-semibold py-2.5 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {loading ? (
